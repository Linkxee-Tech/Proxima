...
function Composer() {
  ...
  const loadCampaignState = async () => {
    try {
      const [tools, savedPosts] = await Promise.all([apiFetch('/api/tools'), apiFetch('/api/social/posts')]);
