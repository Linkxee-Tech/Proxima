...
const sampleImages = [
  ...
  { id: 'sample:workflow', title: 'Workflow overview', category: 'Workflows', src: '/social-gallery/workflow-vertical.png' },
  { id: 'sample:approved', title: 'Human approval', category: 'Trust and safety', src: '/social-gallery/workflow-approved.png' },
];

function Composer() {
  const [goal, setGoal] = useState('Launch our v2 on all social channels');
  ...
  return <><div className="bg-grid" /><main className="shell"><Navbar title="Social Media Hub" /><div className="workspace-page"><Sidebar /><section className="panel page-content"><p className="eyebrow">AI content studio</p><h1>Draft, design, and approve social campaigns</h1><p className="lede">Generate platform-aware copy, upload your own campaign asset, choose a Proxima sample, or generate one with the configured AI image model. Publishing remains approval-first.</p><label className="field"><span>Campaign goal</span><textarea value={goal} onChange={(event) => setGoal(event.target.value)} rows={3} /></label><PlatformSelector selected={platforms} onChange={setPlatforms} /><section className="sample-gallery" aria-label="Proxima sample image gallery"><div className="sample-gallery-header"><div><p className="eyebrow">Sample gallery</p><h2>Choose a campaign image</h2></div><span className="muted">Use one when you do not have an upload yet.</span></div><div className="sample-gallery-grid">{sampleImages.map((sample) => <button type="button" key={sample.id} className={`sample-image-card ${image?.id === sample.id ? 'selected' : ''}`} onClick={() => selectSample(sample)}><img src={sample.src} alt={sample.title} loading="lazy" /><span className="sample-image-meta"><strong>{sample.title}</strong><small>{sample.category}</small></span></button>)}</div></section><div className="social-controls"><label><input type="checkbox" checked={withImage} onChange={(event) => setWithImage(event.target.checked)} /> Generate AI image</label><label className="field"><span>Image prompt</span><input value={imagePrompt} onChange={(event) => setImagePrompt(event.target.value)} disabled={!withImage} /></label><label className="ghost upload-control">Upload image<input type="file" accept="image/png,image/jpeg,image/webp" onChange={upload} hidden /></label>{image ? <button type="button" className="ghost" onClick={() => setImage(null)}>Remove selected image</button> : null}</div><div className="action-row"><button className="primary" disabled={busy || !goal.trim() || !platforms.length} onClick={generate}>Generate campaign</button><label className="field schedule-field"><span>Optional schedule</span><input type="datetime-local" value={scheduledFor} onChange={(event) => setScheduledFor(event.target.value)} /></label><button className="secondary" disabled={busy || !Object.keys(drafts).length} onClick={publish}>{scheduledFor ? 'Schedule selected posts' : 'Send for approval'}</button></div>{message ? <p className="connection-message">{message}</p> : null}<div className="social-hub-grid">{Object.entries(drafts).map(([platform, text]) => <SocialPostCard key={platform} platform={platform} text={text} limit={limits[platform]} image={image} onChange={(key, value) => setDrafts((current) => ({ ...current, [key]: value }))} />)}</div></section></div></main></>;
}
