...
function Preferences() {
  ...
  return <><div className="bg-grid" /><main className="shell"><Navbar title="Settings" /><div className="workspace-page"><Sidebar /><section className="panel page-content"><div className="settings-heading"><div><p className="eyebrow with-icon"><Icon name="settings" size={14} /> Your preferences</p><h1>Settings</h1></div></div><p className="lede">Manage your account, appearance, and security. Your connected apps have their own simple workspace.</p>{message ? <p className="connection-message"><Icon name="info" size={15} /> {message}</p> : null}<div className="resource-list"><article className="resource-card"><p className="eyebrow">General</p><strong>{email || 'Loading account…'}</strong><p className="muted">This is the email address you use to sign in.</p></article><article className="resource-card"><p className="eyebrow">Appearance</p><strong>{theme === 'dark' ? 'Dark mode' : 'Light mode'}</strong><p className="muted">Choose the workspace appearance that is most comfortable for you.</p><div className="action-row"><button className="secondary with-icon" onClick={toggleTheme}><Icon name={theme === 'dark' ? 'spark' : 'moon'} size={15} /> Use {theme === 'dark' ? 'light' : 'dark'} mode</button></div></article><article className="resource-card"><p className="eyebrow">Security</p><strong>Password reset</strong><p className="muted">Request a one-time email link to choose a new password.</p><div className="action-row"><Link className="secondary with-icon" href="/forgot-password"><Icon name="lock" size={15} /> Reset password</Link></div></article><article className="resource-card"><p className="eyebrow">Connected Apps</p><strong>Apps you’ve approved</strong><p className="muted">Connect or remove Google, Slack, social platforms, and other external services.</p><div className="action-row"><Link className="primary with-icon" href="/dashboard/integrations"><Icon name="plug" size={15} /> Manage connected apps</Link></div></article></div></section></div></main></>;
}
