...
function Inner({ kind }) {
  ...
  const approveFromModal = async (options = {}) => {
    if (!approvalToReview) return;
    try {
      if (approvalToReview.socialDrafts) {
        const campaign = await apiFetch('/api/social/publish', { method: 'POST', body: JSON.stringify({ content: approvalToReview.socialDrafts, platforms: Object.keys(approvalToReview.socialDrafts), account_ids: options.accountIds || {}, scheduled_for: options.scheduledFor || null, schedule_timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC', whatsapp_recipient: options.whatsappRecipient || null, approved: !options.scheduledFor }) });
        if (campaign.status === 'failed') throw new Error(Object.values(campaign.results || {}).find((item) => item.status === 'failed')?.error || 'Campaign publishing failed.');
      }
      await act(approvalToReview, 'approve');
      setApprovalToReview(null);
    } catch (err) { setError(err.message); }
  };
